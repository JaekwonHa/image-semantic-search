...
def evaluation(model, x_test, y_test, model_pkl_gcs_uri, acc_csv_gcs_uri, min_acc_progress):
    ...
    print("\bWriting metrics file: /mlpipeline-metrics.json")
    try:
    ...
    if ((acc_score-latestacc) >= min_acc_progress):
        ...
        print("Not meet deploy condition (currnet_acc - latestacc >= min_acc_progress):", str(acc_score), "-", str(latestacc), ">=", min_acc_progress)


def divideGCSUri(gcsUri):
    gcsprefix = "gs://"
    assert(gcsUri.index(gcsprefix) == 0)
    bucket = gcsUri[gcsUri.index(gcsprefix):gcsUri.index("/", len(gcsprefix))]
